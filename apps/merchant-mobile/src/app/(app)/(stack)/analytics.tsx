  ...
  return (
    <SafeAreaView className="flex-1 bg-surface">
      ...
      <Animated.View
        ...
        className="flex-row items-center justify-between px-gutter-mobile py-4 bg-surface border-b border-border-subtle"
      >
        <TouchableOpacity onPress={() => router.back()} className="p-2 -ml-2 rounded-full active:scale-95 min-w-[48px] min-h-[48px] justify-center">
          <MaterialIcons name="storefront" size={24} color="#006B5E" />
        </TouchableOpacity>
        <Text className="font-headline-md text-headline-md font-bold text-growth-green">Sales Analytics</Text>
              ...
              <>
                ...
                <View className="flex-row justify-between px-1 mt-1">
                  {chartBars.map((d, i) => (
                    <Text
                      key={i}
                      className={`flex-1 text-center font-label-sm text-label-sm ${
                        i === chartBars.length - 1 ? 'text-primary font-bold' : 'text-outline'
                      }`}
                    >
